...
| 김가현 | Backend | 설문지 작성/옵션/질문 관련 API, Figma(화면정의서 작성) |
| 차영준 | Backend | 회원가입/로그인, JWT Token, Figma(화면정의서 작성) |
| 하태호 | Frontend | 설문지 생성, 미리보기, 설문 작성, 설문수정, 서비스 로고, Figma(화면정의서 작성) |
| 송윤혁 | Frontend | 로그인, 회원가입, 설문지 목록, Figma(화면정의서 작성) |
| 장유민 | Frontend | 설문 배포(이메일), 설문 결과(통계), Figma(화면정의서 작성) |
