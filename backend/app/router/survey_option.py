...
async def create_options(user_id:int, survey_id:int, question_id:int, option:SurveyOptionCreate, db:AsyncSession=Depends(get_db)):
    new_option=await SurveyOptionService.add_option(db, survey_id,question_id, user_id, option)
...
async def delete_options(user_id:int, survey_id:int, question_id:int, option_id:int, db:AsyncSession=Depends(get_db)):
    deleted=await SurveyOptionService.delete_option(db, survey_id, question_id, option_id, user_id)
    if deleted==0:
