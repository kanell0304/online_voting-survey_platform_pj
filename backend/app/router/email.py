...
async def send_test_email(test_request: EmailTestRequest):
    ...
    return {"성공 여부": True, "msg": message, "받는사람": test_request.recipient_email, "설문지 링크": survey_link}
...
async def delete_email_log_by_id(log_id: int, db: AsyncSession = Depends(get_db)):
    deleted_log_info = await email_crud.get_email_log_by_id(db, log_id)
    success = await email_crud.delete_email_log(db, log_id)
    if not success:
        raise HTTPException(status_code=404, detail="Email Log Not Found")
    return {"msg": "Delete Success Email Log", "deleted_log_info": deleted_log_info.model_dump()}
...
async def send_survey_emails(survey_id: int, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    result = {
            ...
            "survey_link": "", # 설문지 링크
            "success_emails": [], # 발송 성공 이메일 정보
            "failed_emails": [] # 발송 실패 이메일 정보
        }

    # 백그라운드에서 이메일 발송
    async def send_emails_task():
        result = await email_service.send_bulk_emails(email_logs, survey_id)
    ...
    return {
        ...
        "success_count": result["success_count"],  # 발송 성공 개수
        "failed_count": result["failed_count"],  # 발송 실패 개수
        "survey_link": survey_link,  # 설문지 링크
        "success_emails": result["success_emails"],  # 발송 성공 이메일 정보
        "failed_emails": result["failed_emails"]  # 발송 실패 이메일 정보
    }
...
@router.post("/create_logs_and_send", response_model=dict)
async def create_and_send_emails(bulk_data: EmailLogsBulkCreate, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "total": len(bulk_data.recipient_email),
        "survey_link": survey_link,
