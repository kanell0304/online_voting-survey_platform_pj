...
async def create_surveys(new_survey:SurveyCreate, user:int, db:AsyncSession=Depends(get_db)):
    return await SurveyService(db).create_survey(user_id=user, new_survey=new_survey)
