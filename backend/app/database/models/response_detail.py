...
class ResponseDetail(Base):
    ...
    response_id: Mapped[int] = mapped_column(ForeignKey("responses.id", ondelete="CASCADE"), nullable=False) # 설문응답 번호
    question_id: Mapped[int] = mapped_column(ForeignKey("survey_questions.question_id"), nullable=False) # 질문 타입 번호(어떤 질문 타입인지 ex: 텍스트, 다지선다)
