    ...
    <div className="min-h-screen flex justify-center py-12">
      <div className="w-full max-w-2xl bg-blue-100 shadow-lg rounded-2xl p-8 border border-gray-200">
        ...
        <div className='p-5 bg-gray-100 border border-gray-300 mb-5'>
          <h1 className="text-3xl p-1.5 font-semibold mb-3 text-gray-800">{form.title || "제목 없음"}</h1>
          <p className="space-y-3 p-1.5 text-gray-800 text-sm ">{form.description || ""}</p>
        </div>
          ...
          {form.questions.map((q, i)=>(
            <div key={i} className="p-5 bg-gray-50 rounded-xl border border-gray-300 hover:border-blue-400 transition">
              <div className="mb-2 text-gray-800">
                {i + 1}. {q.question_text}{q.is_required && <span className="text-rose-500 ml-1 text-xs align-super">*</span>}
              </div>

              {/* 단답형 */}
              {q.question_type === "short_text" && (
                <input value={answers[i] || ""} placeholder="답변을 입력하세요." onChange={(e)=>setAnswer(i, e.target.value)} className="w-full px-3 py-2 border-b-2 border-gray-400 text-sm bg-transparent outline-none focus:border-blue-500 transition" />
              )}
